...
public:
    ExeNavActionServer(const std::string &node_name):Node(node_name){
        // 创建原生乌龟位姿订阅方，回调函数中获取乌龟位姿
        pose_sub = this->create_subscription<turtlesim::msg::Pose>("/turtle1/pose",10,std::bind(&ExeNavActionServer::poseCallback,this,_1)); 
        // 创建原生乌龟速度发布方
...
private:
    ...
    void execute(std::shared_ptr<rclcpp_action::ServerGoalHandle<Nav>> goal_handle){
        ...
        while (true){
            ...
            // 当两乌龟距离小于0.15米时，将当前乌龟位姿设置进result并退出循环
            if(distance<0.15){
                // 将当前乌龟坐标赋值给result
...
int main(int argc, char* argv[]){
    ...
    auto node = std::make_shared<ExeNavActionServer>("exe_nav_action_server");
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}